...
- **CAPTCHA is automatically disabled for modal mode** (`ContactController.php:74-76`)
...
- The `action_name` for CAPTCHA is set to `'contact'` (semantic identifier) instead of the generic `'homepage'` (`ContactType.php:147`)

**Spam Protection (Version 6.0.2+)**:
- **Honeypot field** is always active for both standard and modal modes (`ContactType.php:133-143`)
- Hidden field named `website` that legitimate users never see or fill
- Bots typically auto-fill all form fields, triggering the honeypot
- Silent rejection: Bots receive success message but email is not sent (`ContactController.php:80-101`)
- Combined with server-side validation for effective spam protection without user friction
